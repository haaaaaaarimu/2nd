...
def make_block(prefix, rect_start, circ_start, df):
    ...
    for row in range(2):
        for col in range(7):
            ...
            # 💡 가장 안전한 title 속성 사용 (레이아웃 파괴 0%)
            tooltip = f"📍위치: {code}\\n🌾곡종: {name}\\n📦재고: {qty}t"
            rects_html += f'<div class="rect-item" title="{tooltip}"><div class="text-box"><span class="p-name" style="color:{p_c}">{name}</span><span class="p-qty" style="color:{q_c}">{qty}</span><span class="p-code">{code}</span></div></div>'
    ...
    for r_idx, y_pos in enumerate([0, 160, 320]):
        for c_idx in range(6):
            ...
            # 💡 가장 안전한 title 속성 사용
            tooltip = f"📍위치: {code}\\n🌾곡종: {name}\\n📦재고: {qty}t"
            circles_html += f'<div class="circle-item" style="top: {y_pos}px; left: {x_pos}px;" title="{tooltip}"><div class="text-box"><span class="p-name" style="color:{p_c}">{name}</span><span class="p-qty" style="color:{q_c}">{qty}</span><span class="p-code">{code}</span></div></div>'
...
if raw_data.strip():
    try:
        ...
        <style>
            ...
            .text-box {{ display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; height: 100%; position: absolute; }}
